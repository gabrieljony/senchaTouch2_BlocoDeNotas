Ext.define('SenchaTouch2-Seminario.controller.Main',{
	...
	showPreenchimentoDoFormulario: function(view, index, target, record, evt, opts){
		if(record && record.data){
			...
			this.getTituloPreenchimentoDoFormulario().setTitle('Atualizar Cadastro');
			this.getBtnExcluirCadastro().show();
		}else{
			this.getPreenchimentoDoFormulario().reset();
			this.getPreenchimentoDoFormulario().setRecord(null);
			this.getTituloPreenchimentoDoFormulario().setTitle('Novo Cadastro');
	...
	salvarNovoCadastro: function(button){
		var form = this.getPreenchimentoDoFormulario();
			cadastro = Ext.create('SenchaTouch2-Seminario.model.Formulario', this.getPreenchimentoDoFormulario().getValues());
